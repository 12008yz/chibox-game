...
async function fixWeightsFromEqual() {
  try {
    ...
    // Функция расчета веса с учетом рентабельности 30% для сайта
    // Веса пересчитаны для поддержания баланса при максимальном бонусе 20%
    function calculateCorrectWeight(price) {
      price = parseFloat(price) || 0;

      // Еще более консервативные веса для обеспечения 30% рентабельности
      // При максимальном бонусе 20% шансы дорогих предметов увеличатся, но не критично
      if (price >= 50000) return 0.002;     // 0.2% - легендарные (было 0.5%)
      if (price >= 30000) return 0.005;     // 0.5% - мифические (было 0.8%)
      if (price >= 20000) return 0.008;     // 0.8% - эпические (было 1.5%)
      if (price >= 15000) return 0.012;     // 1.2% - очень редкие (было 2.5%)
      if (price >= 10000) return 0.02;      // 2% - редкие (было 4%)
      if (price >= 8000) return 0.03;       // 3% - необычные+ (было 6%)
      if (price >= 5000) return 0.05;       // 5% - необычные (было 10%)
      if (price >= 3000) return 0.12;       // 12% - обычные+ (было 20%)
      if (price >= 1000) return 0.25;       // 25% - обычные (было 35%)
      if (price >= 500) return 0.35;        // 35% - частые (было 50%)
      if (price >= 100) return 0.55;        // 55% - очень частые (было 70%)
    ...
    console.log(`\n📊 СИСТЕМА БОНУСОВ:`);
    console.log(`   🎯 Максимальный бонус пользователя: 20%`);
    console.log(`   📈 Подписка: до 8% (уровень 3 = защита от дубликатов)`);
    console.log(`   🏆 Достижения: до 10%`);
    console.log(`   📊 Уровень: до 2% (0.02% за уровень)`);
    console.log(`   💰 Рентабельность: 30% для сайта, 70% для игроков`);
...
async function calculateWeightsByPrice() {
  try {
    ...
    function calculateCorrectWeight(price) {
      ...
      // Еще более консервативные веса для обеспечения 30% рентабельности
      if (price >= 50000) return 0.002;     // 0.2% - легендарные
      if (price >= 30000) return 0.005;     // 0.5% - мифические
      if (price >= 20000) return 0.008;     // 0.8% - эпические
      if (price >= 15000) return 0.012;     // 1.2% - очень редкие
      if (price >= 10000) return 0.02;      // 2% - редкие
      if (price >= 8000) return 0.03;       // 3% - необычные+
      if (price >= 5000) return 0.05;       // 5% - необычные
      if (price >= 3000) return 0.12;       // 12% - обычные+
      if (price >= 1000) return 0.25;       // 25% - обычные
      if (price >= 500) return 0.35;        // 35% - частые
      if (price >= 100) return 0.55;        // 55% - очень частые
      return 1.0;                           // 100% - базовые/дешевые
    }
