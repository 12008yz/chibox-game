      ...
      try {
        ...
        if (newDaysLeft === 3) {
          ...
          await createNotification(
            ...
            'Ваша подписка истекает завтра! Продлите её прямо сейчас.',
            'warning',
            'subscription',
        ...
        if (newDaysLeft === 0) {
          ...
          await createNotification(
            user.id,
            '❌ Подписка истекла',
            'Ваша подписка истекла. Приобретите новую подписку, чтобы снова получать ежедневные кейсы и бонусы к дропу!',
            'error',
            'subscription',
