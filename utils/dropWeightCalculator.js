...
function calculateWeightForPremium499(price) {
  ...
  // СУПЕР ДЖЕКПОТЫ (0.15% общий) - ОЧЕНЬ редкие крупные выигрыши
  if (price >= 50000) return 0.005;     // 0.0005% - ультра мега джекпот (x100)
  if (price >= 30000) return 0.01;      // 0.001% - супер мега джекпот (x60)
  if (price >= 20000) return 0.02;      // 0.002% - мега джекпот++ (x40)
  if (price >= 15000) return 0.05;      // 0.005% - мега джекпот+ (x30)
  if (price >= 10000) return 0.1;       // 0.01% - мега джекпот (x20)
  if (price >= 8000) return 0.2;        // 0.02% - огромный джекпот (x16)
  if (price >= 6000) return 0.3;        // 0.03% - очень большой джекпот (x12)
  if (price >= 5000) return 0.5;        // 0.05% - большой джекпот (x10)
  if (price >= 4000) return 0.8;        // 0.08% - крупный джекпот (x8)
  if (price >= 3000) return 1.2;        // 0.12% - джекпот (x6)
  if (price >= 2500) return 2;          // 0.2% - мини джекпот+ (x5)
  if (price >= 2000) return 3;          // 0.3% - мини джекпот (x4)
  if (price >= 1500) return 5;          // 0.5% - отличный выигрыш (x3)

  // ХОРОШИЕ ВЫИГРЫШИ (2% общий) - удвоение и больше
  if (price >= 1200) return 8;          // 0.8% - x2.4 выигрыш
  if (price >= 1000) return 12;         // 1.2% - x2 выигрыш

  // СРЕДНИЕ ВЫИГРЫШИ (3% общий) - выше стоимости кейса
  if (price >= 800) return 15;          // 1.5% - x1.6 выигрыш
  if (price >= 600) return 20;          // 2% - x1.2 выигрыш
  if (price >= 500) return 25;          // 2.5% - окуп+ (x1)
...
function calculateWeightForStandard99(price) {
  ...
  // СУПЕР ДЖЕКПОТЫ (0.3% общий) - очень редкие крупные выигрыши
  if (price >= 10000) return 0.01;      // 0.001% - ультра джекпот (x100)
  if (price >= 8000) return 0.02;       // 0.002% - мега джекпот++ (x80)
  if (price >= 6000) return 0.05;       // 0.005% - мега джекпот+ (x60)
  if (price >= 5000) return 0.08;       // 0.008% - мега джекпот (x50)
  if (price >= 4000) return 0.15;       // 0.015% - огромный джекпот (x40)
  if (price >= 3000) return 0.3;        // 0.03% - большой джекпот (x30)
  if (price >= 2000) return 0.6;        // 0.06% - крупный джекпот (x20)
  if (price >= 1500) return 1;          // 0.1% - джекпот (x15)
  if (price >= 1000) return 2;          // 0.2% - мини джекпот (x10)
  if (price >= 800) return 3;           // 0.3% - отличный выигрыш (x8)
  if (price >= 500) return 5;           // 0.5% - хороший выигрыш (x5)
