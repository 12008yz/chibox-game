...
function calculateCorrectWeightByPrice(price) {
  ...
  // АГРЕССИВНАЯ оптимизация для достижения 22.5% рентабельности (снижение дорогих предметов на 50-60%)
  if (price >= 50000) return 0.002;     // 0.2% - легендарные (-60%)
  if (price >= 30000) return 0.004;     // 0.4% - мифические (-50%)
  if (price >= 20000) return 0.007;     // 0.7% - эпические (-53%)
  if (price >= 15000) return 0.012;     // 1.2% - очень редкие (-52%)
  if (price >= 10000) return 0.02;      // 2.0% - редкие (-50%)
  if (price >= 8000) return 0.03;       // 3.0% - необычные+ (-50%)
  if (price >= 5000) return 0.06;       // 6.0% - необычные (-40%)
  if (price >= 3000) return 0.12;       // 12% - обычные+ (-40%)
  if (price >= 1000) return 0.28;       // 28% - обычные (-20%)
  if (price >= 500) return 0.45;        // 45% - частые (-10%)
  if (price >= 100) return 0.8;         // 80% - очень частые (+14%)
  return 1.2;                           // 120% - базовые/дешевые (+20%)
}
