 ...
 *
 * НОВАЯ СИСТЕМА ВЕСОВ - ПРИБЫЛЬНАЯ ДЛЯ КАЗИНО
 * Распределение для кейса 499 чикоинов:
 * - 50% кейсов: предметы 150-250 (убыток 50-70%)
 * - 40% кейсов: предметы 400-600 (окуп ±20%)
 * - 8% кейсов: предметы 600-1500 (небольшая прибыль)
 * - 2% кейсов: предметы 1500+ (хорошая прибыль, очень редко)
 */
function calculateCorrectWeightByPrice(price) {
  price = parseFloat(price) || 0;

  // ЭКСТРЕМАЛЬНО РЕДКИЕ ПРЕДМЕТЫ (шанс < 0.01%)
  if (price >= 100000) return 0.001;   // ~0.0001% - ультра мифические (1 из 1000000)
  if (price >= 50000) return 0.005;    // ~0.0005% - легендарные (1 из 200000)
  if (price >= 30000) return 0.01;     // ~0.001% - легендарные+ (1 из 100000)
  if (price >= 20000) return 0.02;     // ~0.002% - эпические (1 из 50000)
  if (price >= 15000) return 0.05;     // ~0.005% - эпические+ (1 из 20000)
  if (price >= 10000) return 0.1;      // ~0.01% - очень редкие (1 из 10000)

  // ОЧЕНЬ РЕДКИЕ ПРЕДМЕТЫ (0.01% - 0.1%)
  if (price >= 8000) return 0.2;       // ~0.02% - очень редкие+ (1 из 5000)
  if (price >= 5000) return 0.3;       // ~0.03% - редкие (1 из 3333)
  if (price >= 3000) return 0.5;       // ~0.05% - редкие+ (1 из 2000)
  if (price >= 2000) return 0.8;       // ~0.08% - необычные++ (1 из 1250)
  if (price >= 1500) return 1.2;       // ~0.12% - необычные++ (1 из 833)

  // РЕДКИЕ ПРЕДМЕТЫ (0.5% - 2%)
  if (price >= 1200) return 3;         // ~0.3% - необычные+ (1 из 333)
  if (price >= 1000) return 5;         // ~0.5% - необычные (1 из 200)
  if (price >= 800) return 8;          // ~0.8% - обычные++ (1 из 125)
  if (price >= 600) return 15;         // ~1.5% - обычные+ (1 из 67)

  // ОКУП КЕЙСА ~40% (предметы 400-600)
  if (price >= 400) return 120;        // ~12% - окуп кейса (примерно 4 из 10)

  // СРЕДНИЙ УБЫТОК ~30% (предметы 250-400)
  if (price >= 250) return 80;         // ~8% - средний убыток (примерно 3 из 10)

  // БОЛЬШОЙ УБЫТОК ~50% (предметы 150-250)
  if (price >= 150) return 150;        // ~15% - большой убыток (примерно 5 из 10)

  // МУСОРНЫЕ ПРЕДМЕТЫ (меньше 150)
  if (price >= 100) return 200;        // ~20% - мусор (1 из 5)
  if (price >= 50) return 250;         // ~25% - очень дешевые
  return 300;                          // ~30% - базовые/дешевые предметы
}
