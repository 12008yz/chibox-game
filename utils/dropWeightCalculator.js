 ...
 *
 * ЦЕЛИ:
 * 1. Казино прибыльно при любом объеме открытий (10% прибыли)
 * 2. Игрокам интересно играть (высокий азарт, частые окупы)
 ...
 *
 * ЭКОНОМИКА:
 * - Все кейсы: RTP 90% → после продажи ~58.5% (казино получает ~10% прибыли)
...
/**
 * ═══════════════════════════════════════════════════════════════════════════
 * СИСТЕМА: РЕНТАБЕЛЬНОСТЬ 10% (RTP 90%)
 * ═══════════════════════════════════════════════════════════════════════════
 * Игрок выигрывает предметы на 90% от цены кейса
 * При продаже за 65%: игрок получает ~58.5% обратно
 * Прибыль казино: 10%
 *
 * СТРАТЕГИЯ АЗАРТА:
 * - 25% шанс: мелкие предметы (40-70% от цены) - частые проигрыши
 * - 25% шанс: средние предметы (70-100% от цены) - "почти окуп"
 * - 35% шанс: хорошие предметы (100-200% от цены) - окуп и выигрыш
 * - 15% шанс: джекпоты (200-1000%+ от цены) - большой выигрыш
 * ═══════════════════════════════════════════════════════════════════════════
 */
 ...
 * ЦЕЛЬ: RTP ~90% для азартной игры
 */
function calculateWeightForPaidCase(price, casePrice) {
  price = parseFloat(price) || 0;
  const ratio = price / casePrice; // Во сколько раз предмет дороже кейса

  // МЕГА ДЖЕКПОТЫ (0.3% общий) - x20+ от цены кейса - УВЕЛИЧЕНО x3
  if (ratio >= 50) return 0.03;         // 0.003% - x50+ (ультра джекпот)
  if (ratio >= 30) return 0.06;         // 0.006% - x30-50
  if (ratio >= 20) return 0.15;         // 0.015% - x20-30

  // СУПЕР ДЖЕКПОТЫ (1.2% общий) - x10-20 от цены - УВЕЛИЧЕНО x3
  if (ratio >= 15) return 0.24;         // 0.024% - x15-20
  if (ratio >= 10) return 0.45;         // 0.045% - x10-15
  if (ratio >= 8) return 0.75;          // 0.075% - x8-10

  // БОЛЬШИЕ ДЖЕКПОТЫ (4.5% общий) - x4-8 от цены - УВЕЛИЧЕНО x3
  if (ratio >= 6) return 1.5;           // 0.15% - x6-8
  if (ratio >= 5) return 2.4;           // 0.24% - x5-6
  if (ratio >= 4) return 3.6;           // 0.36% - x4-5

  // СРЕДНИЕ ДЖЕКПОТЫ (9% общий) - x2-4 от цены - УВЕЛИЧЕНО x3
  if (ratio >= 3) return 9;             // 0.9% - x3-4
  if (ratio >= 2.5) return 15;          // 1.5% - x2.5-3
  if (ratio >= 2) return 24;            // 2.4% - x2-2.5

  // ХОРОШИЕ ВЫИГРЫШИ (15% общий) - x1.5-2 от цены - УВЕЛИЧЕНО x3
  if (ratio >= 1.8) return 36;          // 3.6% - x1.8-2
  if (ratio >= 1.6) return 45;          // 4.5% - x1.6-1.8
  if (ratio >= 1.4) return 54;          // 5.4% - x1.4-1.6
  if (ratio >= 1.2) return 36;          // 3.6% - x1.2-1.4

  // ОКУП И ВЫШЕ (20% общий) - x1-1.2 от цены (АЗАРТ!) - УВЕЛИЧЕНО
  if (ratio >= 1.1) return 60;          // 6% - x1.1-1.2
  if (ratio >= 1.0) return 70;          // 7% - точный окуп
  if (ratio >= 0.95) return 70;         // 7% - почти окуп

  // БЛИЗКО К ОКУПУ (25% общий) - x0.7-0.95
  if (ratio >= 0.9) return 80;          // 8% - x0.9-0.95
  if (ratio >= 0.8) return 90;          // 9% - x0.8-0.9
  if (ratio >= 0.7) return 80;          // 8% - x0.7-0.8

  // СРЕДНИЙ УБЫТОК (20% общий) - x0.4-0.7 - УМЕНЬШЕНО
  if (ratio >= 0.6) return 70;          // 7% - x0.6-0.7
  if (ratio >= 0.5) return 70;          // 7% - x0.5-0.6
  if (ratio >= 0.4) return 60;          // 6% - x0.4-0.5

  // БОЛЬШОЙ УБЫТОК (5% общий) - x0.2-0.4 - СИЛЬНО УМЕНЬШЕНО
  if (ratio >= 0.3) return 25;          // 2.5% - x0.3-0.4
  if (ratio >= 0.2) return 25;          // 2.5% - x0.2-0.3

  // МУСОР (меньше 20% от цены) - СИЛЬНО УМЕНЬШЕНО
  if (ratio >= 0.1) return 15;          // 1.5% - x0.1-0.2
  return 10;                            // 1% - меньше x0.1
}
