...
function calculateCorrectWeightByPrice(price) {
  ...
  // Улучшенная система весов с более агрессивным снижением для дорогих предметов
  // Это позволяет добавлять дорогие предметы в кейсы без ущерба рентабельности
  if (price >= 100000) return 0.05;    // ~0.001% - ультра мифические (1 из 100000)
  if (price >= 50000) return 0.1;      // ~0.002% - легендарные (1 из 50000)
  if (price >= 30000) return 0.2;      // ~0.005% - легендарные+ (1 из 20000)
  if (price >= 20000) return 0.3;      // ~0.01% - эпические (1 из 10000)
  if (price >= 15000) return 0.5;      // ~0.02% - эпические+ (1 из 5000)
  if (price >= 10000) return 0.8;      // ~0.03% - очень редкие (1 из 3000)
  if (price >= 8000) return 1.2;       // ~0.05% - очень редкие+ (1 из 2000)
  if (price >= 5000) return 2;         // ~0.1% - редкие (1 из 1000)
  if (price >= 3000) return 2.5;       // ~0.12% - редкие+ (1 из 800) - снижен для кейса 99₽
  if (price >= 2000) return 3;         // ~0.15% - необычные++ (1 из 650) - снижен для кейса 99₽
  if (price >= 1500) return 4;         // ~0.2% - необычные++ (1 из 500) - снижен для кейса 99₽
  if (price >= 1000) return 6;         // ~0.3% - необычные+ (1 из 330) - снижен для кейса 99₽
  if (price >= 500) return 10;         // ~0.5% - необычные (1 из 200) - снижен для кейса 99₽
  if (price >= 300) return 20;         // ~1% - обычные++ (1 из 100) - снижен для кейса 99₽
  if (price >= 200) return 35;         // ~1.8% - обычные+ (1 из 55) - снижен для кейса 99₽
  if (price >= 150) return 50;         // ~2.5% - обычные (1 из 40) - снижен для кейса 99₽
  if (price >= 100) return 70;         // ~3.5% - частые+ (1 из 28) - снижен для кейса 99₽
  if (price >= 50) return 200;         // ~15% - очень частые (1 из 6)
  return 250;                          // ~20% - базовые/дешевые (1 из 5)
}
