class TicTacToeService {
  ...
  makeBotMove(board) {
    // 1. Проверяем, можем ли выиграть (только 60% времени)
    const winningMove = this.getWinningMove(board);
    if (winningMove !== null && Math.random() < 0.6) {
      return winningMove;
    }

    // 2. Проверяем, нужно ли блокировать игрока (только 40% времени)
    ...
    return this.getBestMove(board, 0.25); // 25% сложность - очень легкий бот
  }

  // Создаем новую игру
  createNewGame(userGoesFirst = true) {
    const botGoesFirst = Math.random() < 0.25; // 25% шанс что бот ходит первым
